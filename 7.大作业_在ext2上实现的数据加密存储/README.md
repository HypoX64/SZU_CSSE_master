...
```
&emsp;&emsp;在程序执行中```ret = filp->f_op->read_iter(&kiocb, &iter);```将文件操作指向子文件系统，下面将以ext2为例进行分析。进入```‘/fs/ext2/file.c:194’```可以看到:
```c
//fs/ext2/file.c
...
......
```
&emsp;&emsp;```generic_file_read_iter()```与```generic_file_write_iter()```均位于’/mm/filemap.c'中，然后```generic_file_read_iter()```将调用```do_generic_file_read()```.
...
......
```
&emsp;&emsp;然后在```generic_file_read_iter_my()```以及```generic_file_write_iter_my()```中调用原本的函数并将传出的数据截获并进行加密以及解密就可以了。
```c
//  my_modules/ext2/file.c
...
```
&emsp;&emsp;由于这个结构体不包含读写文件的真实数据信息，所以这个不需要进行处理，直接传入传出就可以。
&emsp;&emsp;下面将重点分析```struct iov_iter *```结构体，这里存放了读写数据的指针。进入```struct iov_iter *```的定义，说明文字都附在源码后面。
...
&emsp;&emsp;同时修改在写入时中插入printk(KERN_ALERT "in ext2");便于后面测试模块是否被成功调用
```c
//  my_modules/ext2/file.c
ssize_t generic_file_write_iter_my(struct kiocb *iocb, struct iov_iter *from)
{
...
[root@localhost ext2]# 
```
截图如下，观察发现写入的```bbbbbbb```被截获并编码为了```ccccccc```
即对明文加密写入而不解密就读出的话，将不能获得明文。
计算验证：
b->114(ascii)->111 0010
...
[4] 清水浊酒.iov_iter结构体--翻译.https://blog.csdn.net/C_JLMPC2009/article/details/102597790.
[5] Linux kernel Function Tree. http://www.bricktou.com/idx_root_tree.html.
[6]qljessica123. 模块化编译ext2, ext3,ext4 文件系统.https://blog.csdn.net/qljessica123/article/details/38010579.
